...
ToxinKeyMatch <- read.delim('../ToxinKeyMatch/combined_output_keywords.tsv', sep = '\t', stringsAsFactors = FALSE)
colnames(df)

hhmer_orfs <- read.csv('../hhmer_tx_VenomZone/hhmer_Tx_orf_mapping.csv')
colnames(hhmer_orfs)

library(dplyr)

# Step 1.1 Left join to add `ToxinFamily` column from `hhmer_orfs` to `df`
merged <- ToxinKeyMatch %>%
...
df <- df %>%
  mutate(FiltDeTox_Classification = case_when(
    # First Condition: If the wolfpsort_prediction does NOT contain 'extr' or 'E.R.', classify as 'Non-Toxins'
    !grepl("extr|E.R.", wolfpsort_prediction) ~ 'Non-Toxins',
    
    # Second Condition: If ToxinFamily is not NA, classify as follows:
    !is.na(ToxinFamily) & ORF_precursor_length > 500 ~ 'Unlikely-Toxins',  # ORF_precursor_length > 500
    !is.na(ToxinFamily) & grepl("[*]", Rating) ~ 'Unlikely-Toxins',  # Matching on `*` in Rating
    !is.na(ToxinFamily) ~ 'Toxins-Candidates',  # Remaining cases with ToxinFamily
    
    # Third Condition: Additional classifications for strong toxin candidates
    Rating %in% c('SBCD', 'SBCDT', 'SBC', 'SBCT') & ORF_precursor_length > 500 ~ 'Unlikely-Toxins',  # Long sequences
    Rating %in% c('SBCD', 'SBCDT', 'SBC', 'SBCT') ~ 'Toxins-Candidates',  # Strong toxin candidates based on flags
    
    # Fourth Condition: Remaining sequences with TRUE in both toxin_keywords and pfam_ToxinKeywords
    toxin_keywords == 'TRUE' & pfam_ToxinKeywords == 'TRUE' & ORF_precursor_length > 500 ~ 'Non-Toxins',  # Long sequences
    toxin_keywords == 'TRUE' & pfam_ToxinKeywords == 'TRUE' ~ 'Unlikely-Toxins',  # Default for TRUE keywords
    
    # Fifth Condition: Handle specific BD, B, and SBD cases
    Rating %in% c('*BD', '*B', 'BD', 'B', 'SBD') & ORF_precursor_length > 500 ~ 'Non-Toxins',  # Long sequences
    Rating %in% c('*BD', '*B', 'BD', 'B', 'SBD') ~ 'Unlikely-Toxins',  # Default for these ratings
    
    # Sixth Condition: Rating is SC
    Rating == 'SC' & (grepl('uncharacterized', tolower(hit_descr)) | hit_descr == "" | is.na(hit_descr)) ~ 'SCRs-WA',  # Handle uncharacterized hits
    Rating == 'SC' ~ 'Non-Toxins',  # Default for SC not matching the above conditions
...
# Remove labels from the tree object directly
tree$tip.label <- rep("", length(tree$tip.label))  # Set all tip labels to empty strings
...
p_dotplot_combined <- ggplot(pfam_data_sorted, aes(x = 1, y = pfam.domains)) +
  ...
  scale_color_viridis_c(option = "C", trans = "log",labels = scales::number_format(accuracy = 0.01))+  # Log scale for color based on TPM, showing two decimal places
...
# Optional: Save the combined plot in .pdf
ggsave("Dendrogram_and_DotPlot_ORFs_TPM_by_ORF.pdf", plot = combined_plot, width = 14, height = 8)

# Print completion message
print("Combined dendrogram and dot plot saved as 'Dendrogram_and_DotPlot_ORFs_TPM_by_ORF.pdf'.")


### Generating FASTA Files with Precursor and Mature Peptides for Further Analyses such as complex networks for in-Silico Bioactivity Analysis
# Load necessary library
library(dplyr)

# Function to create FASTA with mature peptides
generate_mature_fasta <- function(tsv_file, fasta_file_name) {
  ...
  fasta_data <- data %>%
    ...
    filter(!is.na(mature_peptide))  # Ensure there are no missing sequences
  ...
  close(fasta_file)
  print(paste("Mature peptide FASTA file '", fasta_file_name, "' created successfully.", sep = ""))
}

# Function to create FASTA with full precursor sequences
generate_precursor_fasta <- function(tsv_file, fasta_file_name) {
  data <- read.delim(tsv_file, sep = '\t', header = TRUE, stringsAsFactors = FALSE)
  
  fasta_data <- data %>%
    select(ID, Sequence) %>%
    filter(!is.na(Sequence))  # Ensure there are no missing sequences
  ...
  close(fasta_file)
  print(paste("Precursor sequence FASTA file '", fasta_file_name, "' created successfully.", sep = ""))
}

# Generate both mature and precursor FASTA files for each classification, including SCRs-WA
